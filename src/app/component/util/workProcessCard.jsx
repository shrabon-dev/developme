...
export default function WorkProcessCard({icon, title, description}) {
  return (
    <>
    <div className="w-full md:w-[48%] lg:w-[48%] group hover:bg-theme-main duration-300  hover:rotate-[5deg] ease-in-out transition-all card bg-theme-bg p-5 lg:p-10 rounded-3xl flex flex-col items-start justify-start gap-4 ">
       <div className="title flex items-center justify-between w-full pb-5 lg:pb-10">
        <h5 className='text-xs lg:text-lg font-sportB text-theme-text bg-theme-secondary rounded-full px-4 py-1 group-hover:bg-theme-bg duration-300 ease-in-out transition-colors group-hover:text-theme-light-text'>{title}</h5>
        <div className="flex gap-2 items-center">
          ...
          <Link className='text-xs lg:text-lg font-sportR text-theme-light-text border-b border-theme-light-brdr duration-300 transition-colors group-hover:border-theme-brdr group-hover:text-theme-text' href="/"> Read More</Link>
        </div>
      </div>
